	...
	#pragma omp for
     ...
     for (int i=0; i<NCA_NRB; i++) 
      ...
      for (dest=1; dest<=numworkers; dest++)
      {
         ...
         MPI_Send(&A[offset][0], rows*NCA_NRB, MPI_INT, dest, mtype,
                   MPI_COMM_WORLD);
         MPI_Send(&B, NCA_NRB*NCB, MPI_INT, dest, mtype, MPI_COMM_WORLD);         
         offset = offset + rows;
      }
...
/**************************** worker task ************************************/
   if (taskid > MASTER)
   {
      ...
      MPI_Recv(&A, rows*NCA_NRB, MPI_INT, MASTER, mtype, MPI_COMM_WORLD, &status);
      MPI_Recv(&B, NCA_NRB*NCB, MPI_INT, MASTER, mtype, MPI_COMM_WORLD, &status);
